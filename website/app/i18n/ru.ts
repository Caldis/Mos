import type { Translations } from "./context";

export const ru: Translations = {
  donate: {
    trigger: "Поддержать Mos",
    footerLink: "Поддержать",
    title: "Поддержать Mos",
    intro: "Mos бесплатна и с открытым исходным кодом — и так будет всегда. Чаевые совершенно не обязательны, но я буду очень рад.",
    paypal: "PayPal",
    buyMeACoffee: "Buy Me a Coffee",
    qrLabel: "Материковый Китай",
    alipay: "Alipay",
    wechat: "WeChat Pay",
    scanHint: "Откройте {app} и отсканируйте",
    meowWall: "Хотите посмотреть фотки кота?",
  },
  languageSelector: {
    title: "Выбор языка",
  },
  a11y: {
    skipToContent: "Перейти к содержимому",
    closeDialog: "Закрыть диалог",
    githubAria: "Mos на GitHub",
    appIconAlt: "Иконка Mos",
    appProfileIconAlt: "Иконка {app} для профиля прокрутки Mos по приложениям",
    scrollCurveGraph: "График кривой прокрутки",
  },
  nav: {
    githubTitle: "GitHub",
  },
  hero: {
    badgeLine1: "Плавная прокрутка колесом мыши в macOS",
    badgeLine2: "профили по приложениям · независимые оси · кнопки и шорткаты",
    titleLine1: "Твоя мышь,",
    titleLine2Before: "",
    titleLine2Highlight: "заново",
    titleLine2After: ".",
    lead:
      "Колесо следует за пальцами, как трекпад, а каждая кнопка мыши раскрывает весь свой потенциал.",
    ctaDownload: "Скачать Mos",
    ctaViewGitHub: "Открыть на GitHub",
    ctaInstallHomebrew: "Установить через Homebrew",
    requirementsLine1: "Нужна macOS 10.13+",
    requirementsLine2: "Бесплатно · Открытый код",
    scrollHint: "Прокрутите вниз",
  },
  sectionFeel: {
    title: "Предсказуемая прокрутка. Настраиваемое ощущение.",
    lead:
      "Mos превращает сырые дельты колеса в предсказуемое движение. Сохраняйте одинаковое ощущение в разных приложениях и включайте исключения там, где нужно.",
    cards: {
      curves: {
        kicker: "Кривые и ускорение",
        title: "Сформируйте ощущение.",
        body:
          "Плавность — это кривая. Меняйте шаг, усиление и длительность и смотрите, как сырые дельты превращаются в контролируемое движение.",
      },
      axes: {
        kicker: "Независимые оси",
        title: "Разделите X и Y.",
        body:
          "Вертикальная и горизонтальная прокрутка идут как отдельные оси. Плавность и реверс можно включать и выключать независимо для каждой.",
        smooth: "Плавность",
        reverse: "Реверс",
        on: "Вкл",
        off: "Выкл",
      },
      perApp: {
        kicker: "Профили по приложениям",
        title: "Разным приложениям — разное ощущение.",
        body:
          "Для каждого приложения — своя кривая прокрутки и поведение. Точно или плавно — решаешь ты.",
      },
      buttons: {
        kicker: "Кнопки и шорткаты",
        title: "Привязать, записать, повторить.",
        body:
          "Привяжи мышь или клавиатуру к быстрым действиям и выстраивай рабочий процесс как угодно.",
        quickBind: "Быстрая привязка",
        rows: {
          button4: "Кнопка 4",
          button5: "Кнопка 5",
          wheelClick: "Нажатие колеса",
          missionControl: "Mission Control",
          nextSpace: "Следующее пространство",
          appSwitcher: "Переключатель приложений",
        },
      },
    },
  },
  download: {
    title: "Поставь Mos — и мышь слушается тебя.",
    body:
      "Устанавливается за секунды. Настраивайте, когда понадобится, и держите одинаковое поведение прокрутки в тех приложениях, где вы живете.",
    ctaDownload: "Скачать",
    releaseNotes: "Заметки к релизу",
    docs: "Документация",
  },
  homebrew: {
    title: "Homebrew",
    copy: "Копировать",
    copied: "Скопировано",
    tip: "Тестовую сборку можно поставить через {cask}.",
  },
  footer: {
    latestRelease: "Последний релиз",
    latestVersion: "Последний {version}",
    requiresMacos: "Поддерживает macOS 10.13 и новее",
    github: "GitHub",
    wiki: "Wiki",
    language: "Язык",
    releases: "Релизы",
    about: "О Mos",
    compare: "Сравнить",
    privacy: "Конфиденциальность",
  },
  easing: {
    graphAria: "График кривой прокрутки",
    step: {
      label: "Шаг",
      aria: "Шаг",
      help: "Нижний предел квантования для дельт колеса.",
    },
    gain: {
      label: "Усиление",
      aria: "Усиление",
      help: "Масштабирует расстояние за тик и скорость разгона кривой.",
    },
    duration: {
      label: "Длительность",
      aria: "Длительность",
      help: "Константа сглаживания (выше — длиннее хвост).",
    },
    footer: "Кривая ScrollCore",
  },
  scroll: {
    kickerFeel: "Ощущение",
    kickerProfiles: "Профили",
    kickerButtons: "Кнопки",
    heading: "Не читайте — прокрутите.",
    lead:
      "Обычное колесо мыши опрашивается не так часто, как трекпад, поэтому страница может лишь дёргаться рывками. Mos переосмысляет этот ввод тщательно настроенным алгоритмом на основе реальной физики, превращая ваше намерение в плавное и точное движение, и страница течёт за вашими пальцами.",
    withoutMos: "Без Mos",
    withMos: "С Mos",
    tagRaw: "сырое",
    tagSmooth: "плавно",
    scrollInside: "прокрутите здесь",
    parameters: "параметры",
  },
  wall: {
    back: "Mos",
    title: "Стена",
    tagline: "оставь стикер",
    empty: "будь первым, кто оставит заметку",
    trayHint: "Перетащи стикер на стену",
    trayDragAria: "Перетащи {color} стикер на стену",
    bodyPlaceholder: "Напиши что-нибудь…",
    namePlaceholder: "Твоё имя (необязательно)",
    colorAria: "цвет {color}",
    cancel: "Отмена",
    submit: "Прилепить ↗",
    submitting: "Лепим…",
    anonymous: "аноним",
    delete: "Удалить заметку",
    deleteConfirm: "Удалить?",
    verifyHint: "Секунда — проверим, что ты человек…",
    errorGeneric: "Не удалось отправить заметку. Попробуй ещё раз.",
    errorRate: "Слишком быстро — попробуй через минуту.",
    errorTurnstile: "Пройди проверку и попробуй снова.",
    errorLinks: "В заметке слишком много ссылок.",
  },
};
